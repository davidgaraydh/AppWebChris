      ...
      <div class="col-lg-5"    >
        <div class="card">
           ...
           <div class="col-lg-12" >
            ...
            <input type="text" class="form-control form-control-rounded" min="1" max="1000" id="idticket" placeholder="Ingrese su Ticket">
              </div>
              <div class="col-lg-6" >
   ...
   <div class="col-lg-6" >
           ...
           <div class="form-group">
                      ...
                      <div class="col-lg-12">
                        <div class="row" >
                          <div class="col-lg-6" >
                                        <label for="input-9">Acciones</label>
            <input type="number" class="form-control form-control-rounded" min="1" max="100000" id="idacciones" placeholder="Ingrese sus Acciones">
                          </div>
                           <div class="col-lg-6" >
                                         <label for="input-6">Utilidad</label>
            <input type="number" disabled="disabled" class="form-control form-control-rounded" min="1" max="1000" id="idutilidad" placeholder="Ingrese su utilidad">
                           </div>
                        </div>
           ...
           <div class="col-lg-12" >
            ...
            <select class="form-control form-control-rounded" id="idsetup" >
              ...
              <option value="Venta" >Venta</option>
                ...
                <div class="col-lg-6" >
                              <label for="input-8">Inversion</label>
            <input type="number"  disabled="disabled" class="form-control form-control-rounded" min="1" max="1000" id="idinversion" placeholder="Calculo de Inversion">
                </div>
             </div>
           </div> 
                     <div class="form-group text-center mt-3">
            <a onclick="SaveFuturos();"  id="btnSave" data-id="Guardar" class="btn btn-light btn-round px-5"><i class="icon-lock"></i> Guardar</a>
          </div>
          </form>
         </div>
         </div>
    ...
    <thead style="font-size: 10px;color: #ff5400" >
        <tr>
          <th>Ticket</th>
            <th>Entrada</th>
            <th>Salida</th>
...
$resultado=0;
  ...
  if(($("#identrada").val().length!=0) && ($("#idacciones").val().length!=0)){
   var entrada= Number($("#identrada").val());
   var salida= Number($("#idsalida").val());
      var acciones= Number($("#idacciones").val());


$resultado=(salida-entrada)*acciones;


 $otrocalculo=Number($("#identrada").val())*Number($("#idacciones").val());

$("#idinversion").val($otrocalculo);
...
.then((willDelete) => {
  if (willDelete) {
                    ...
                    $.ajax({  
                url: "Core/acciones/acciones.php",
                type: "post",  
...
location.reload(true);
 ...
 var form_data = new FormData(document.getElementById('frm_acciones')); 
...
$("#idacciones").val(data["acciones"]);
$("#idutilidad").val(data["utilidad"]);
$("#idsetup").val(data["setup"]);
$("#idinversion").val(data["inversion"]);
$("#btnSave").attr("data-id",data["idAcciones"]);
...
$("#btnSave").attr("id","Actualizar");
    ...
    $('#TablaAcciones').DataTable({
          "ajax": "Core/acciones/acciones.php?Accion=LoadTable",
    "deferRender": true,
  "retrieve": true,
  "processing": true,
     "language": {
 ...
 var form_data = new FormData(document.getElementById('frm_acciones'));                  
    $ticket=$("#idticket").val();
     $stop=$("#idstop").val();
       ...
       $salida=$("#idsalida").val();
       $acciones=$("#idacciones").val();
        $utilidad=$("#idutilidad").val();
       $setup=$("#idsetup").val();
$inversion=$("#idinversion").val();
 ...
 form_data.append('ticket', $ticket);
        ...
        form_data.append('entrada', $entrada);
         form_data.append('salida', $salida);
          form_data.append('acciones', $acciones);
        form_data.append('utilidad', $utilidad);
        form_data.append('setup', $setup);
         form_data.append('inversion', $inversion);
